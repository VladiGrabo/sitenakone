...
export default function GoalsSection({ userId, currency, onUpdate }: GoalsSectionProps) {
  ...
  const calculateTimeToGoal = (goal: Goal) => {
    ...
    if (monthsNeeded < 12) return `${monthsNeeded} ${monthsNeeded < 5 ? 'months' : 'months'}`;

    const years = Math.floor(monthsNeeded / 12);
    const months = monthsNeeded % 12;

    if (months === 0) {
      return years === 1 ? '1 year' : `${years} ${years < 5 ? 'years' : 'years'}`;
    }

    const yearText = years === 1 ? 'year' : years < 5 ? 'years' : 'years';
    const monthText = months === 1 ? 'month' : months < 5 ? 'months' : 'months';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 sm:mb-4 gap-2">
        <h2 className="text-lg sm:text-2xl font-bold text-slate-900">Financial Goals</h2>
        <button
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
        <div className="flex gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xs sm:text-sm text-blue-900 font-medium mb-1">Why is it important to set financial goals?</p>
            <p className="text-xs sm:text-sm text-blue-800">
              Clear financial goals help our system develop a personalized action plan, calculate necessary savings amounts, and suggest optimal timelines for achievement. Without goals, it's impossible to build an effective financial strategy and track progress.
            </p>
          </div>
        </div>
      </div>
            ...
            <div key={goal.id} className="bg-white border-2 border-slate-200 rounded-lg sm:rounded-xl p-4 sm:p-6 hover:shadow-xl transition-all hover:border-emerald-300">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
                <div className="bg-slate-50 rounded-lg p-2.5 sm:p-3">
                  <span className="text-xs text-slate-600 block mb-1">Current</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs sm:text-sm">
                  ...
                  <span className="text-slate-600 truncate">
                    Until <span className="font-medium text-slate-900">{new Date(goal.deadline).toLocaleDateString('en-GB')}</span>
                  </span>
                </div>
                ...
                {remaining > 0 && (
                  <div className="flex items-center gap-2 text-xs sm:text-sm bg-blue-50 rounded-lg p-2 sm:p-2.5 border border-blue-200">
                    ...
                    <span className="text-blue-700 font-medium truncate">
                      Remaining £{remaining.toLocaleString('en-GB')}
                    </span>
                  </div>
                )}
              </div>
      ...
      {goals.length === 0 && !showForm && (
        <div className="text-center py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-300">
          ...
          <p className="text-slate-600 text-lg font-medium">No financial goals yet</p>
          <p className="text-sm text-slate-500 mt-2">Add your first goal to start planning</p>
        </div>
      )}
    </div>
  );
