...
const EXPENSE_CATEGORIES = ['Food', 'Transport', 'Housing', 'Entertainment', 'Health', 'Education', 'Shopping', 'Other'];
...
export default function ExpensesSection({ userId, currency, onUpdate }: ExpensesSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const title = formData.category === 'Other' ? formData.title : formData.category;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 sm:mb-4 gap-2">
        <h2 className="text-lg sm:text-2xl font-bold text-slate-900">Expenses</h2>
        <button
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
        <div className="flex gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xs sm:text-sm text-blue-900 font-medium mb-1">Why is it important to record all expenses?</p>
            <p className="text-xs sm:text-sm text-blue-800">
              Detailed expense tracking allows our system to identify areas for budget optimization, find hidden costs, and suggest ways to save. This is the foundation for building an effective financial plan and achieving your goals faster.
            </p>
          </div>
        </div>
      </div>
            ...
            {formData.category === 'Other' && (
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all"
                  placeholder="For example: Groceries at store"
                />
              </div>
            )}
      ...
      {expenses.length === 0 && !showForm && (
        <div className="text-center py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-300">
          ...
          <p className="text-slate-600 text-lg font-medium">No expense records yet</p>
