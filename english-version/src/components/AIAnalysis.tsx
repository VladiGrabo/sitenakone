...
export default function AIAnalysis({ userId, stats, netWorth, goals, currency }: AIAnalysisProps) {
  ...
  const formatMonths = (months: number) => {
    ...
    if (years > 0 && remainingMonths > 0) {
      const yearWord = getPluralForm(years, 'year', 'years', 'years');
      const monthWord = getPluralForm(remainingMonths, 'month', 'months', 'months');
      return `${years} ${yearWord} ${remainingMonths} ${monthWord}`;
    } else if (years > 0) {
      const yearWord = getPluralForm(years, 'year', 'years', 'years');
      return `${years} ${yearWord}`;
    } else {
      const monthWord = getPluralForm(months, 'month', 'months', 'months');
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-slate-600">Analysisируем ваши финансы...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-600">Personalized assessment of your financial situation</p>
        </div>
      </div>
        ...
        <div className={`border-2 rounded-xl p-6 ${getHealthColorClass(analysis.healthColor)}`}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-bold capitalize">
              Financial Health: {analysis.metrics.financialHealth}
            </h3>
          </div>
      ...
      <div className="bg-gradient-to-br from-white to-slate-50 border-2 border-slate-200 rounded-xl p-6 shadow-lg">
        ...
        {(() => {
          ...
          const metricsStartIndex = lines.findIndex(line => line.includes('**Overall Picture:**'));
          ...
          const getMetricIcon = (label: string) => {
            if (label.includes('capital')) return { icon: '💰', color: 'from-emerald-500 to-teal-600' };
            if (label.includes('income')) return { icon: '📈', color: 'from-blue-500 to-cyan-600' };
            if (label.includes('expense')) return { icon: '💳', color: 'from-orange-500 to-amber-600' };
            if (label.includes('asset')) return { icon: '🏦', color: 'from-purple-500 to-indigo-600' };
            if (label.includes('savings')) return { icon: '🎯', color: 'from-pink-500 to-rose-600' };
      ...
      {analysis.goalProjections && analysis.goalProjections.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-xl text-slate-900">Analysis достижения целей</h3>
          </div>
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-semibold text-slate-900">
                      {new Date(projection.deadline).toLocaleDateString('en-GB')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className={`border-2 rounded-lg p-4 ${projection.currentScenario.achievable ? 'bg-green-50 border-green-300' : 'bg-red-50 border-red-300'}`}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h5 className="font-bold text-sm">With Current Income</h5>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                    <div>
                      <span className="text-slate-700">Time Required:</span>
                      <span className="ml-2 font-semibold">{formatMonths(projection.currentScenario.monthsNeeded)}</span>
                    </div>
                    <div>
                      <span className="text-slate-700">By Deadline:</span>
                      <span className={`ml-2 font-semibold ${projection.currentScenario.achievable ? 'text-green-700' : 'text-orange-700'}`}>
                        {projection.currentScenario.achievable ? '✓ Achievable' : 'Difficult under current conditions'}
                      </span>
                    </div>
                    <div className="sm:col-span-2">
                      <span className="text-slate-700">Need to Save:</span>
                      <span className="ml-2 font-semibold">
                        {formatAmount(projection.currentScenario.monthlySavingsRequired, currency)}/month
                      </span>
                    </div>
                  </div>
                </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
        <p className="text-sm text-slate-600 text-center">
          Analysis updates automatically when data changes
        </p>
      </div>
    </div>
  );
