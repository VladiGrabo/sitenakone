...
const ASSET_TYPES = ['Недвижимость', 'Автомобиль', 'Инвестиции', 'Сбережения', 'Криптовалюта', 'Ценные бумаги', 'Другое'];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3 sm:mb-4 gap-2">
        <h2 className="text-lg sm:text-2xl font-bold text-slate-900">Активы</h2>
        <button
        ...
        >
          ...
          <span className="hidden xs:inline">Добавить актив</span>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 mb-4 sm:mb-6">
        <div className="flex gap-2 sm:gap-3">
          ...
          <div>
            <p className="text-xs sm:text-sm text-blue-900 font-medium mb-1">Почему важно указывать все активы?</p>
            <p className="text-xs sm:text-sm text-blue-800">
              Полный учет активов помогает нашей системе оценить вашу чистую стоимость, определить потенциал для инвестиций и составить долгосрочную стратегию роста капитала. Понимание всех ваших ресурсов критично для грамотного финансового планирования.
            </p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-6 mb-6 border border-slate-200 shadow-md">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-2">
                Текущая стоимость ({getCurrencySymbol(currency)})
              </label>
              <input
                type="number"
      ...
      {assets.length === 0 && !showForm && (
        <div className="text-center py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-300">
          ...
          <p className="text-slate-600 text-lg font-medium">Пока нет записей об активах</p>
          <p className="text-sm text-slate-500 mt-2">Добавьте первый актив для отслеживания</p>
        </div>
      )}
    </div>
  );
